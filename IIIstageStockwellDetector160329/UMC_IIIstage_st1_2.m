...
function [HFOobj] = UMC_IIIstage_st1_2(Signal, HFOobj)
...
%     % STAGE I
    ...
    HFOobj.CDFlevel  = 0.99;        %input.CDFlevel; %.99 percentile of detected baselines, incr= incr in trheshold, 
    ...
    % ------STAGE I-------------------------------------------------------- 
    [HFOobj] = baselinethreshold(Signal, Signalfilt, env, HFOobj);
    HFOobj   = findEOI(env,HFOobj,Signalfilt);
    stageItoc = toc
    tic
    % ------STAGE II------------------------------------------------------- 
    HFOobj   = events_validation(Signal, Signalfilt, HFOobj);
    stageIItoc = toc
...
function [signalfilt, HFOobj] = filter_and_param(Signal, HFOobj)
    ...
    switch HFOobj.hp
        case 80  % Ripples
            ...
            HFOobj.lf_bound = 60;
            HFOobj.hf_bound = 250;
            HFOobj.Ampl_bound = 500*2;
            HFOobj.time_thr = 0.015*HFOobj.fs;
...
function [HFOobj] = baselinethreshold(sigfull, sigfiltered, env, HFOobj)
    ...
    indHighEntr=[];
    S(HFOobj.fs)=0;
%     HFOobj.maxNoisemuV = 30*std(sigfiltered);
    ...
    for sec=1:floor(HFOobj.dur) % calculate ST for every second
        ...
        % DEBUG
        %         ecco = find(diff(indAboveThr)>1)
        %         clear ax
        %         close all
        %         figure,
        %         [b,a] = butter(2,[80, 500]/1000)
        %         sugfilt = filtfilt(b,a,signal);
        %         ax(1)=subplot(2,1,1), plot(sugfilt)
        %         for ii = 1:length(ecco)-1
        %             inty = indAboveThr(ecco(ii)+1:ecco(ii+1)-1);
        %             hold on, plot(inty,sugfilt(inty),'r')
        %         end
        % %         subplot(312),  imagesc(t,f, stda, Clim)
        % %             set(gca,'YDir','normal');
        %         ax(2)=subplot(2,1,2),  plot(S)
        %         for ii = 1:length(ecco)-1
        %             inty = indAboveThr(ecco(ii)+1:ecco(ii+1)-1);
        %             hold on, plot(inty,S(inty),'r')
        %         end
        %         line([0 2000],[thr thr],'color','k')
        %         linkaxes(ax,'x')
...
%                             end 
    ...
    HFOobj.THRsss(HFOobj.ch)                = thrCDF;
    HFOobj.THR                              = thrCDF;
    HFOobj.indHighEntr(HFOobj.ch).indexes   = indHighEntr;  
    HFOobj.b_mean(HFOobj.ch)                = mean(baseline);
    HFOobj.b_sd (HFOobj.ch)                 = std(baseline);
...
function HFOobj = findEOI(env,HFOobj,signalfilt) 
    ...
    env(1)                  = 0; 
    env(length(env))        = 0; % assign the first and last positions at 0 point
...
function HFOobj  = events_validation(Signal, Signalfilt, HFOobj)
...
%                     title(num2str(trig_start(triggy)/2048))
        ...
        HFOobj.EVENTS = [HFOobj.EVENTS;  HFOobj.ch HFOobj.trig_start(triggy) HFOobj.aboveTHR(triggy),...
